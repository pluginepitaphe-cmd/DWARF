"""
Run C — DSQG Hybrid 13M @ N=2048, Interleaved Dense+Sparse Offset Set
=======================================================================

What changes vs condU 13M @ N=2048 baseline (52.206 PPL, 43.3% passkey):
  - Offset set redesigned: interleaved dense windows + sparse long-range (J=40)
  - pos_bias init: unchanged (linspace, same as baseline) — isolates offset effect
  - All other hyperparameters identical

Offset design (J=40):
  Dense window 1: δ=0,...,15   (16 offsets) — local coherence
  Dense window 2: δ=32,...,48  (17 offsets) — fills the gap at δ=33-47
  Sparse long:    {64,128,256,512,768,1024,1536}  (7 offsets)

Motivation:
  condU 35M pure DSQG (epochs 5-9): d=32 passkey stuck at 0% for 5 consecutive
  epochs while d=16 reaches 60%. The retrieval cue is ~4 tokens wide, so
  d=32 requires attending to δ≈36, which falls in the V3 gap at δ=33-47.
  Dense window 2 covers δ=32-48 directly.

Trade-off vs V3 (linspace-comparison, not near-zero init):
  Lost: δ=16-31 (16 dense offsets from V3)
  Gained: δ=32-48 (17 dense offsets, fills the gap)
  The full-attention layer at position 5 compensates for DSQG gaps in the hybrid.
  Expected: improved passkey at d=32+; possible degradation at d=16 without full-attn help.

Kernel: kernels/dsqg_attention_v3_interleaved.py (J=40, max δ=1536 unchanged)
  All kernel changes documented in that file. The only Triton changes are:
    tl.static_range(40) instead of (44), and the updated offset tuple.

Run:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 -u train/dsqg_hybrid_13m_2048_interleaved.py \\
    2>&1 | tee logs/dsqg_hybrid_13m_2048_interleaved_run.log

Before running: smoke-test the kernel:
  CUDA_VISIBLE_DEVICES=0 .venv/bin/python3 kernels/dsqg_attention_v3_interleaved.py
"""
...
# J=40 interleaved offset set
_DENSE_1        = list(range(16))           # δ=0-15
_DENSE_2        = list(range(32, 49))       # δ=32-48 (fills gap)
_SPARSE         = [64, 128, 256, 512, 768, 1024, 1536]
_COND_C_OFFSETS = sorted(set(_DENSE_1) | set(_DENSE_2) | set(_SPARSE))
NUM_OFFSETS     = len(_COND_C_OFFSETS)      # 40
assert NUM_OFFSETS == 40
...
from dsqg_attention_v3_interleaved import dsqg_attention_v3_interleaved

def dsqg_attention_backend(q, k, v, pos_bias, scale_embed):
    return dsqg_attention_v3_interleaved(q, k, v, pos_bias, scale_embed)
...
class DSQGAttentionQW(nn.Module):
    def __init__(self, embedding_dim, num_heads, seq_len=2048, dropout=0.1):
        ...
        # Run C: linspace init (same as condU 13M baseline) — ONLY offset set changes.
        # This isolates the interleaved offset effect from the init effect (Run A).
        alphas     = torch.linspace(0.2, 2.0, num_heads)
        delta_vals = torch.tensor([math.log(1.0 + d) for d in _COND_C_OFFSETS],
                                  dtype=torch.float32)
...
def train(model, train_data, val_data, test_data, tokenizer, device='cuda'):
    ...
    print(f'\n  Run C (interleaved) TEST PPL: {test_ppl:.3f}')
    ...
    results = {
        'experiment':      'run_c_dsqg_hybrid_13m_2048_interleaved',
        'description':     'Interleaved dense+sparse offsets (J=40); N=2048; linspace init',
        'offset_set':      _COND_C_OFFSETS,
        'offset_design':   'dense 0-15 | gap | dense 32-48 | sparse 64-1536',
        'change_vs_base':  'offset set only; init unchanged (linspace)',
...
def main():
    ...
    print('=' * 70)
    print('  Run C — DSQG Hybrid 13M @ N=2048 — Interleaved Offset Set (J=40)')
    print(f'  D={EMBEDDING_DIM}, H={NUM_HEADS}, L={NUM_LAYERS}, N={MAX_SEQ_LEN}')
    print(f'  Offsets ({NUM_OFFSETS}): dense 0-15 | dense 32-48 | sparse 64-1536')
    print(f'  vs V3: lost δ=16-31, gained δ=32-48 (gap fill)')
    print('=' * 70)
